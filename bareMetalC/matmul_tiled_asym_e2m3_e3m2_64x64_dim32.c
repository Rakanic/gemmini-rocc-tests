...
#include "include/matmul_data_asym_e2m3_e3m2_dim32.h"
...
int main() {
  ...
  // config_ex: A (act) = fp6/E2M3, B (wgt) = fp6/E3M2, out = BF16, uselut=1.
  // Per-operand altfmt: rs1[6]=mx_fp8_altfmt=1 (act E2M3), rs1[31]=weight_altfmt_diff=1
  // -> weight altfmt = 1 XOR 1 = 0 = E3M2.
  ...
  // Both LUTs real, loaded at 6-bit: A_lut (sel=1) = E2M3 codebook, B_lut (sel=0) = E3M2 codebook.
  ...
  if (errors == 0) printf("e2m3xe3m2 asym WS matmul test PASSED (no mismatches).\n");
  else             printf("e2m3xe3m2 asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
