...
#include "include/matmul_data_asym_e2m3_e4m3.h"
...
int main() {
  ...
  // config_ex: A (act) = fp6/E2M3 (code1), B (wgt) = fp8/E4M3 (code0), out = BF16, uselut=1.
  // mode9 dual-quad (both sig4). Per-operand altfmt: rs1[6]=mx_fp8_altfmt=1 (act E2M3),
  // rs1[31]=weight_altfmt_diff=1 -> weight altfmt = 1 XOR 1 = 0 = E4M3.
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,
    ...
    | ((uint64_t)(0) << 12)         // B (weight) format = fp8 (code0); wgt altfmt=0 -> E4M3
  ...
  // Both LUTs real, loaded 8-bit: A_lut (sel=1) = E2M3 codebook, B_lut (sel=0) = E4M3 codebook.
  // (E2M3's 6-bit codes sit in the low 6 bits of 8-bit slots; spike/RTL slice low 6.) C_lut (sel=2)
  ...
  if (errors == 0) printf("e2m3xe4m3 asym WS matmul test PASSED (no mismatches).\n");
  else             printf("e2m3xe4m3 asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
