...
#include "include/matmul_data_asym_e2m3_e5m2_128x128x256_dim32.h"

#define TILE 16
#define DIM 32
#define VALUES_PER_BYTE 2
#define USE_LUT 1
#define MX_ALTFMT 1                 // config_ex rs1[6]=mx_fp8_altfmt (activation altfmt)
...
int main() {
  ...
  // config_ex: A (act) = fp6/E2M3, B (wgt) = fp8/E5M2, out = BF16, uselut=1.
  // Per-operand altfmt: rs1[6]=mx_fp8_altfmt=1 (act E2M3), rs1[31]=weight_altfmt_diff=0
  // -> weight altfmt = 1 XOR 0 = 1 = E5M2.
  ...
  // Both LUTs real, loaded at 8-bit: A_lut (sel=1) = E2M3 codebook, B_lut (sel=0) = E5M2 codebook.
  // (E2M3/E3M2 6-bit codes are packed as a 8-bit stream; spike/RTL slice low 6.) C_lut (sel=2)
  ...
  if (errors == 0) printf("e2m3xe5m2 asym WS matmul test PASSED (no mismatches).\n");
  else             printf("e2m3xe5m2 asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
