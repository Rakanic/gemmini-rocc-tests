// Asymmetric FP6_E2M3-activation (quad LUT) x FP4-weight 64x64 matmul, BF16 output — MxAsymE2M3Fp4GemminiRocketConfig.
// A (activation) = E2M3 via LUT (6-bit codebook; fp6/code1 + rs1[6]=mx_fp8_altfmt=1 -> E2M3 sub-format,
// uselut=1 quad path). B (weight) = FP4 E2M1 DIRECT 4-bit codes (no LUT). Output BF16 (out_mx_fmt=3).
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
#include "include/matmul_data_asym_e2m3_fp4_dim32.h"

#define TILE 16
#define DIM 32
#define VALUES_PER_BYTE 2
#define USE_LUT 1
#define MX_ALTFMT 1                 // config_ex rs1[6]=1: fp6 alt1 -> E2M3
...
int main() {
  ...
  // config_ex: A (act) format = fp6/E2M3 (code1 + altfmt1), B (wgt) format = fp4 (code2, direct),
  // out format = 3 (BF16), uselut = 1 (bit 5), mx_fp8_altfmt = 1 (bit 6 -> E2M3 within fp6 code1).
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,
    ...
    | ((uint64_t)(1) << 10)         // A (activation) format = fp6 (code1); altfmt=1 -> E2M3
  ...
  // A_lut (sel=1) is the real E2M3 codebook (6-bit; activation deproject). B_lut/C_lut are
  // placeholders: fp4 weights take the direct path (no LUT) and the output is bf16. (scale sel 0=A,1=B.)
  ...
  if (errors == 0) printf("e2m3xfp4 asym WS matmul test PASSED (no mismatches).\n");
  else             printf("e2m3xfp4 asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
