// Asymmetric FP8_E3M2-act x FP8/FP6_E4M3-wei 64x64 matmul, BF16 output. Both operands via LUT.
// Per-operand altfmt: rs1[6]=0 (act E3M2); rs1[31]=weight_altfmt_diff -> weight altfmt = 0 XOR 0 = 0 = E4M3. out_mx_fmt=3.
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
#include "include/matmul_data_asym_e3m2_e4m3.h"

#define TILE 16
#define DIM 16
#define VALUES_PER_BYTE 2
#define USE_LUT 1
#define MX_ALTFMT 0                 // config_ex rs1[6]: activation altfmt (E3M2)
...
int main() {
  ...
  // config_ex: A (act) = E3M2, fp8/E4M3 (wgt), per-operand altfmt via rs1[6]/rs1[31],
  // out format = 3 (BF16), uselut = 1 (bit 5). Per-operand altfmt: rs1[6]=mx_fp8_altfmt=1 (act E5M2),
  // rs1[31]=weight_altfmt_diff=1 so weight altfmt = 1 XOR 1 = 0 = E3M2.
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,
      ((uint64_t)acc_scale_t_to_acc_scale_t_bits((acc_scale_t)ACC_SCALE_IDENTITY) << 32)
    | ((uint64_t)(0) << 31)         // weight_altfmt_diff (NEW): weight altfmt = act_altfmt XOR this
    | ((uint64_t)(1) << 16)         // A stride
    | ((uint64_t)(3) << 14)         // C (out) format = BF16
    | ((uint64_t)(0) << 12)         // B (weight) format = fp6 (code1); wgt altfmt=0 -> E3M2
    | ((uint64_t)(1) << 10)         // A (activation) format = fp8 (code0); altfmt=1 -> E5M2
  ...
  // Both LUTs are real and loaded at 8-bit: A_lut (sel=1) = E5M2 codebook, B_lut (sel=0) = E3M2
  // codebook (6-bit codes in the low 6 bits of 8-bit slots; RTL/spike slice low 6). C_lut (sel=2)
  // is an 8-bit placeholder (output is bf16). (scale sel 0=A, 1=B.)
  ...
  if (errors == 0) printf("e3m2xe4m3 asym WS matmul test PASSED (no mismatches).\n");
  else             printf("e3m2xe4m3 asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
