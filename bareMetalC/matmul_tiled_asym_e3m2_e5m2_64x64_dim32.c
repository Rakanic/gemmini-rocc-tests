// Asymmetric FP6_E3M2-activation x FP8_E5M2-weight 64x64 matmul, BF16 output.
// BOTH operands deproject via LUT: A = E3M2 (6-bit codes, loaded 8-bit-padded), B = E5M2 (8-bit).
// PER-OPERAND altfmt (inverse of e5m2xe3m2): rs1[6]=mx_fp8_altfmt=0 (activation E3M2 = alt0), and
// rs1[31]=weight_altfmt_diff=1 so weight altfmt = 0 XOR 1 = 1 = E5M2. Output BF16 (out_mx_fmt=3).
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
#include "include/matmul_data_asym_e3m2_e5m2_dim32.h"

#define TILE 16
#define DIM 32
#define VALUES_PER_BYTE 2
#define USE_LUT 1
#define MX_ALTFMT 0                 // config_ex rs1[6]: activation E3M2 = alt0
...
int main() {
  ...
  // config_ex: A (act) format = fp6/E3M2 (code1 + altfmt0), B (wgt) format = fp8/E5M2 (code0 + wgt altfmt),
  // out format = 3 (BF16), uselut = 1 (bit 5). Per-operand altfmt: rs1[6]=mx_fp8_altfmt=0 (act E3M2),
  // rs1[31]=weight_altfmt_diff=1 so weight altfmt = 0 XOR 1 = 1 = E5M2.
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,
    ...
    | ((uint64_t)(0) << 12)         // B (weight) format = fp8 (code0); wgt altfmt=1 -> E5M2
  ...
  // Both LUTs are real and loaded at 8-bit: A_lut (sel=1) = E3M2 codebook (6-bit codes in the low 6
  // bits of 8-bit slots; RTL/spike slice low 6), B_lut (sel=0) = E5M2 codebook. C_lut (sel=2) is an
  // 8-bit placeholder (output is bf16). (scale sel 0=A, 1=B.)
  ...
  if (errors == 0) printf("e3m2xe5m2 asym WS matmul test PASSED (no mismatches).\n");
  else             printf("e3m2xe5m2 asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
