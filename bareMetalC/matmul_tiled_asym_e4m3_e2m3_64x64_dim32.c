...
#include "include/matmul_data_asym_e4m3_e2m3_dim32.h"
...
int main() {
  ...
  // config_ex: A (act) = fp8/E4M3 (code0), B (wgt) = fp6/E2M3 (code1), out = BF16, uselut=1.
  // mode9 dual-quad (both sig4). Per-operand altfmt: rs1[6]=mx_fp8_altfmt=0 (act E4M3),
  // rs1[31]=weight_altfmt_diff=1 -> weight altfmt = 0 XOR 1 = 1 = E2M3.
  ...
  // Both LUTs real, loaded 8-bit: A_lut (sel=1) = E4M3 codebook, B_lut (sel=0) = E2M3 codebook.
  ...
  if (errors == 0) printf("e4m3xe2m3 asym WS matmul test PASSED (no mismatches).\n");
  else             printf("e4m3xe2m3 asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
