// Asymmetric FP8_E4M3-act x FP8/FP6_E5M2-wei 64x64 matmul, BF16 output. Both operands via LUT.
// Per-operand altfmt: rs1[6]=0 (act E4M3); rs1[31]=weight_altfmt_diff -> weight altfmt = 0 XOR 1 = 1 = E5M2. out_mx_fmt=3.
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
#include "include/matmul_data_asym_e4m3_e5m2_dim32.h"
...
int main() {
  ...
  // config_ex: A (act) = E4M3, fp8/E5M2 (wgt), per-operand altfmt via rs1[6]/rs1[31],
  ...
  if (errors == 0) printf("e4m3xe5m2 asym WS matmul test PASSED (no mismatches).\n");
  else             printf("e4m3xe5m2 asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
