// Asymmetric FP8_E4M3-activation (quad LUT) x FP4-weight 64x64 matmul, BF16 output — MxAsymE4M3Fp4GemminiRocketConfig.
// A (activation) = E4M3 via LUT (8-bit codebook; config_ex rs1[6]=mx_fp8_altfmt=0 selects E4M3 within
// fp8/code0, uselut=1 enables the quad path). B (weight) = FP4 E2M1 DIRECT 4-bit codes (no LUT).
...
#include "include/matmul_data_asym_e4m3_fp4_dim32.h"

#define TILE 16
#define DIM 32
#define VALUES_PER_BYTE 2
#define USE_LUT 1
#define MX_ALTFMT 0                 // config_ex rs1[6]=0: selects E4M3 (not E5M2)
...
int main() {
  ...
  // config_ex: A (act) format = fp8/E4M3 (code0 + altfmt0), B (wgt) format = fp4 (code2, direct),
  ...
  // A_lut (sel=1) is the real E4M3 codebook (activation deproject). B_lut/C_lut are placeholders:
  ...
  if (errors == 0) printf("e4m3xfp4 asym WS matmul test PASSED (no mismatches).\n");
  else             printf("e4m3xfp4 asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
