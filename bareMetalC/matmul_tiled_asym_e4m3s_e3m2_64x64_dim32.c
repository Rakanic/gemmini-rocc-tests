// Asymmetric FP8_E4M3-SINGLE-activation x FP6_E3M2-weight 64x64 matmul, BF16 (mode7, dual throughput).
...
#include "include/matmul_data_asym_e4m3s_e3m2_dim32.h"
...
int main() {
  ...
  // config_ex: A (act) = fp8/E4M3 (code0, altfmt0) DIRECT single. B (wgt) = fp6/E3M2 (code1, altfmt diff 0).
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,
    ...
    | ((uint64_t)(1) << 12)         // B (weight) format = fp6 (code1) -> E3M2 (altfmt diff 0)
  ...
  // Load ONLY the weight LUT (sel=0). No activation LUT: E4M3 act is direct single. Output is BF16.
  ...
  if (errors == 0) printf("e4m3s(single)xe3m2 asym WS matmul test PASSED (no mismatches).\n");
  else             printf("e4m3s(single)xe3m2 asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
