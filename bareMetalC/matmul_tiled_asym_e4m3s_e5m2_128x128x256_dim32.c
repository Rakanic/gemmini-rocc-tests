// Asymmetric FP8_E4M3-SINGLE-activation x FP8_E5M2-weight 64x64 matmul, BF16 (mode7, dual throughput).
// A (activation) = E4M3 DIRECT 8-bit codes, single (1 row/lane, NO act LUT). B (weight) = E3M2 4-bit LUT
// indices, quad (2 col/lane, weight LUT loaded). => 2 products/PE, 16x32 output tile.
...
#include "include/matmul_data_asym_e4m3s_e5m2_128x128x256_dim32.h"

#define DIM 32
#define VALUES_PER_BYTE 2
#define USE_LUT 1                   // a weight LUT is used (E3M2 deproject); act stays single (no act LUT)
#define MX_ALTFMT 0                 // E4M3 within fp8/code0 (act); weight altfmt via rs1[31] diff = 0 -> E3M2
...
int main() {
  ...
  // config_ex: A (act) = fp8/E4M3 (code0, altfmt0) DIRECT single. B (wgt) = fp8/E5M2 (code0, altfmt diff 1 -> E5M2).
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,
    ...
    | ((uint64_t)(1) << 31)         // weight_altfmt_diff: weight altfmt = act_altfmt(0) XOR 1 = 1 -> E5M2
    | ((uint64_t)(0) << 12)         // B (weight) format = fp8 (code0) -> E5M2 via altfmt diff
    ...
    | ((uint64_t)(MX_ALTFMT) << 6)  // mx_fp8_altfmt (act)
  ...
  // Load ONLY the weight LUT (sel=0, E5M2 8-bit codebook). No activation LUT: E4M3 act is direct single. Output is BF16.
  ...
  // MVIN B: E3M2 LUT indices nibble-packed [K][N/2], 32 cols/tile (16 bytes); stride = MATMUL_N/2 bytes.
  ...
  if (errors == 0) printf("e4m3s(single)xe5m2 asym WS matmul test PASSED (no mismatches).\n");
  else             printf("e4m3s(single)xe5m2 asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
