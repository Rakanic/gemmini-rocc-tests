// Asymmetric FP8_E4M3-SINGLE-activation x FP4-weight 128x128 matmul, BF16 output (mode6, dual throughput).
// A (activation) = E4M3 DIRECT 8-bit codes, single throughput (1 row/lane, NO LUT: uselut=0, altfmt=0).
// B (weight)     = FP4 E2M1 DIRECT 4-bit codes, quad (2 col/lane). => 2 products/PE, 16x32 output tiles.
// Output is BF16 (out_mx_fmt=3), read back from the internal scratchpad, compared to golden C_out_bf16.
// Exercises the multi-row-tile (tiles_I=8) AND multi-col-tile (tiles_J=4, max_j=4) act-single spad layout.
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
#include "include/matmul_data_asym_e4m3s_fp4_128x128.h"
...
int main() {
  ...
  if (errors == 0) printf("e4m3s(single)xfp4 128x128 asym WS matmul test PASSED (no mismatches).\n");
  else             printf("e4m3s(single)xfp4 128x128 asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
