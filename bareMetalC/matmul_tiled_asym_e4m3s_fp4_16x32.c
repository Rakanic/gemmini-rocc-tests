// Minimal SINGLE-TILE asymmetric E4M3-single x FP4 matmul (mode6): M=16, K=32, N=32.
// tiles_I=1, tiles_J=1 -> isolates the WITHIN-tile 16x32 output layout (no multi-tile dst_offset stride).
// A (activation) = E4M3 DIRECT 8-bit codes, single throughput (NO LUT). B = FP4 quad (2 col/lane).
// Output BF16 (out_mx_fmt=3), written to internal scratchpad, flat-read back, compared to golden.
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
#include "include/matmul_data_asym_e4m3s_fp4_16x32.h"
...
#define OUT_COLS (MATMUL_N / BF16_PER_WORD)   // columns (N), not M -- N and M differ here
...
int main() {
  ...
  if (errors == 0) printf("e4m3s(single)xfp4 asym 16x32 single-tile test PASSED (no mismatches).\n");
  else             printf("e4m3s(single)xfp4 asym 16x32 single-tile test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
