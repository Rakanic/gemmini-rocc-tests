...
#include "include/matmul_data_asym_e5m2_e2m3_dim32.h"
...
int main() {
  ...
  // config_ex: A (act) = fp8/E5M2, B (wgt) = fp6/E2M3, out = BF16, uselut=1.
  // Per-operand altfmt: rs1[6]=mx_fp8_altfmt=1 (act E5M2), rs1[31]=weight_altfmt_diff=0
  // -> weight altfmt = 1 XOR 0 = 1 = E2M3.
  ...
  // Both LUTs real, loaded at 8-bit: A_lut (sel=1) = E5M2 codebook, B_lut (sel=0) = E2M3 codebook.
  ...
  if (errors == 0) printf("e5m2xe2m3 asym WS matmul test PASSED (no mismatches).\n");
  else             printf("e5m2xe2m3 asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
