...
#include "include/matmul_data_asym_e5m2_e3m2_dim32.h"
...
int main() {
  ...
  // config_ex: A (act) format = fp8/E5M2 (code0 + altfmt), B (wgt) format = fp6/E3M2 (code1 + wgt altfmt),
  ...
  if (errors == 0) printf("e5m2xe3m2 asym WS matmul test PASSED (no mismatches).\n");
  else             printf("e5m2xe3m2 asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
