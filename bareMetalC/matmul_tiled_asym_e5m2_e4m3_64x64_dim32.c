// Asymmetric FP8_E5M2-act x FP8/FP6_E4M3-wei 64x64 matmul, BF16 output. Both operands via LUT.
// Per-operand altfmt: rs1[6]=1 (act E5M2); rs1[31]=weight_altfmt_diff -> weight altfmt = 1 XOR 1 = 0 = E4M3. out_mx_fmt=3.
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
#include "include/matmul_data_asym_e5m2_e4m3_dim32.h"

#define TILE 16
#define DIM 32
#define VALUES_PER_BYTE 2
#define USE_LUT 1
#define MX_ALTFMT 1                 // config_ex rs1[6]: activation altfmt (E5M2)
...
int main() {
  ...
  // config_ex: A (act) = E5M2, fp8/E4M3 (wgt), per-operand altfmt via rs1[6]/rs1[31],
  ...
  if (errors == 0) printf("e5m2xe4m3 asym WS matmul test PASSED (no mismatches).\n");
  else             printf("e5m2xe4m3 asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
