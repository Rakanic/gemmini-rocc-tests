// Asymmetric FP8_E5M2-activation x FP8_E4M3-SINGLE-weight 64x64 matmul, BF16 (mode5, dual throughput 32x16).
...
#include "include/matmul_data_asym_e5m2_e4m3s_dim32.h"

#define DIM 32
#define VALUES_PER_BYTE 2
#define USE_LUT 1                   // activation E3M2 deproject (act LUT). Weight is direct single (no LUT).
#define MX_ALTFMT 1                 // E5M2 within fp8/code0 (act, altfmt1); weight altfmt diff 1 -> E4M3
...
int main() {
  ...
  // config_ex: A (act) = fp8/E5M2 (code0, altfmt1) quad LUT. B (wgt) = fp8/E4M3 (code0, altfmt diff 1 -> E4M3) single.
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,
    ...
    | ((uint64_t)(1) << 31)         // weight_altfmt_diff: weight altfmt = act_altfmt(1) XOR 1 = 0 -> E4M3
  ...
  if (errors == 0) printf("e5m2xe4m3s(single) asym WS matmul test PASSED (no mismatches).\n");
  else             printf("e5m2xe4m3s(single) asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
