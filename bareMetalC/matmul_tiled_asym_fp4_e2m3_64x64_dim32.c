// Asymmetric FP4-activation x FP6_E2M3-weight (quad LUT) 64x64 matmul, BF16 output — MxAsymFp4E2M3GemminiRocketConfig.
// A (activation) = FP4 E2M1 DIRECT 4-bit codes (no LUT). B (weight) = E2M3 via LUT (6-bit codebook,
// fp6/code1 + weight altfmt=1). Output is BF16 (out_mx_fmt=3), read
// back from the internal scratchpad and compared against the golden C_out_bf16.
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
#include "include/matmul_data_asym_fp4_e2m3_dim32.h"

#define TILE 16
#define DIM 32
#define VALUES_PER_BYTE 2
#define USE_LUT 1
#define MX_ALTFMT 0                 // config_ex rs1[6]=0 (act fp4 has no altfmt)
...
int main() {
  ...
  // config_ex: A (act) = fp4 (code2, direct), B (wgt) = fp6/E2M3 (code1 + wgt altfmt), out = BF16, uselut=1.
  // Per-operand altfmt: rs1[6]=0 (fp4 act), rs1[31]=weight_altfmt_diff=1 -> weight altfmt = 0 XOR 1 = 1 = E2M3.
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,
      ((uint64_t)acc_scale_t_to_acc_scale_t_bits((acc_scale_t)ACC_SCALE_IDENTITY) << 32)
    | ((uint64_t)(1) << 31)         // weight_altfmt_diff -> weight = E2M3
    | ((uint64_t)(1) << 16)         // A stride
    | ((uint64_t)(3) << 14)         // C (out) format = BF16
    | ((uint64_t)(1) << 12)         // B (weight) format = fp6 (code1); wgt altfmt=1 -> E2M3
    | ((uint64_t)(2) << 10)         // A (activation) format = fp4 (code2), direct
  ...
  // B_lut (sel=0) is the real E2M3 codebook (6-bit) (weight deproject). A_lut/C_lut are placeholders:
  // fp4 activations take the direct path (no LUT) and the output is bf16. (scale sel 0=A, 1=B.)
  ...
  if (errors == 0) printf("fp4xe2m3 asym WS matmul test PASSED (no mismatches).\n");
  else             printf("fp4xe2m3 asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
