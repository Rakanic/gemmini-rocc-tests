// Asymmetric FP4-activation x FP8_E4M3-weight (quad LUT) 64x64 matmul, BF16 output — MxAsymFp4E4M3GemminiRocketConfig.
// A (activation) = FP4 E2M1 DIRECT 4-bit codes (no LUT). B (weight) = E4M3 via LUT (8-bit codebook,
// config_ex rs1[6]=mx_fp8_altfmt=0 selects E4M3 within fp8/code0, uselut=1 quad path). Output is BF16 (out_mx_fmt=3), read
...
#include "include/matmul_data_asym_fp4_e4m3.h"

#define TILE 16
#define DIM 16
#define VALUES_PER_BYTE 2
#define USE_LUT 1
#define MX_ALTFMT 0                 // config_ex rs1[6]=0: E4M3 (not E5M2)
...
int main() {
  ...
  // config_ex: A (act) format = fp4 (code2, direct), B (wgt) format = fp8/E4M3 (code0 + altfmt0),
  // out format = 3 (BF16), uselut = 1 (bit 5), mx_fp8_altfmt = 0 (bit 6 -> E4M3 within fp8 code0).
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,
    ...
    | ((uint64_t)(0) << 12)         // B (weight) format = fp8 (code0); altfmt=0 -> E4M3
  ...
  // B_lut (sel=0) is the real E4M3 codebook (weight deproject). A_lut/C_lut are placeholders:
  ...
  if (errors == 0) printf("fp4xe4m3 asym WS matmul test PASSED (no mismatches).\n");
  else             printf("fp4xe4m3 asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
