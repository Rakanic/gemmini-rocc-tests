// Asymmetric FP4-activation x FP8_E4M3-SINGLE-weight 64x64 matmul, BF16 (mode2, dual throughput 32x16).
// A (activation) = FP4 4-bit codes, quad (2 rows/lane, HW-tiled, no LUT). B (weight) = E4M3 DIRECT 8-bit,
// single (1 col/lane, no LUT). => 2 products/PE, 32x16 output tile.
...
#include "include/matmul_data_asym_fp4_e4m3s.h"
...
int main() {
  ...
  // config_ex: A (act) = fp4 (code2, direct quad). B (wgt) = fp8/E4M3 (code0, altfmt0) DIRECT single.
  // uselut=0 (no LUT anywhere). out = BF16.
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,
    ...
    | ((uint64_t)(0) << 12)         // B (weight) format = fp8 (code0) -> E4M3 single (altfmt diff 0)
    | ((uint64_t)(2) << 10)         // A (activation) format = fp4 (code2), direct quad
    | ((uint64_t)(0) << 9)          // B transpose
    | ((uint64_t)(0) << 8)          // A transpose
    | ((uint64_t)(0) << 7)          // set only strides
    | ((uint64_t)(MX_ALTFMT) << 6)  // mx_fp8_altfmt
    | ((uint64_t)(0) << 5)          // uselut = 0
  ...
  // No LUTs (FP4 act direct, E4M3 weight direct single, output bf16).
  ...
  // MVIN A: FP4 nibble-packed HW-tiled [M/2][K], 32 rows/tile (16 hw-rows); stride = MATMUL_K bytes.
  ...
  if (errors == 0) printf("fp4xe4m3s(single) asym WS matmul test PASSED (no mismatches).\n");
  else             printf("fp4xe4m3s(single) asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
