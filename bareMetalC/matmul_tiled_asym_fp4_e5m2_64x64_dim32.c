// Asymmetric FP4-activation x FP8_E5M2-weight 64x64 matmul, BF16 output — MxAsymFp4E5M2GemminiRocketConfig.
// A (activation) = FP4 E2M1 DIRECT 4-bit codes (no LUT). B (weight) = E5M2 via LUT (8-bit codebook,
// config_ex rs1[6]=mx_fp8_altfmt selects E5M2 within fp8/code0). Output is BF16 (out_mx_fmt=3), read
...
#include "include/matmul_data_asym_fp4_e5m2_dim32.h"
...
int main() {
  ...
  // config_ex: A (act) format = fp4 (code2, direct), B (wgt) format = fp8/E5M2 (code0 + altfmt),
  // out format = 3 (BF16), uselut = 1 (bit 5), mx_fp8_altfmt = 1 (bit 6 -> 8-bit E5M2 codebook).
  ...
  // B_lut (sel=0) is the real E5M2 codebook (weight deproject). A_lut/C_lut are placeholders:
  ...
  if (errors == 0) printf("fp4xe5m2 asym WS matmul test PASSED (no mismatches).\n");
  else             printf("fp4xe5m2 asym WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
