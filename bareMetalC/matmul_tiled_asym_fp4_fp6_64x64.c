...
// Asymmetric FP4-activation x FP6_E3M2-weight data, generated by gen_asym_fp4_fp6.py
#include "include/matmul_data_asym_fp4_fp6.h"
...
#define FP6
#define DIM 16
// Per-operand MX format codes: activation = FP4 (2), weight = FP6 (1), output = BF16 (3)
#define ACT_MX_FMT 2
#define WGT_MX_FMT 1
#define OUT_MX_FMT 3
...
int main() {
  ...
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,
    ...
    | (WGT_MX_FMT << 12) // B format (FP6)
    | (ACT_MX_FMT << 10) // A format (FP4)
    | (0 << 9) // B transpose
    | (0 << 8) // A transpose
    ...
    | (0),
    k_CONFIG);
  int tiles_I = MATMUL_M / 32;                       // = 2 (64/32)
  int tiles_K = MATMUL_K / 16;                         // = 4 (64/16)
  int tiles_J = MATMUL_N / 32 ;                           // = 2 (64/32, each n-tile = DIM*2 cols)
  ...
  // Asymmetric: act_mx_fmt=FP4(2), wgt_mx_fmt=FP6(1), out_mx_fmt=BF16(3), uselut=1
  ...
  // B (weight) LUT is the one that matters (fp6 deproject). A_lut/C_lut are placeholders:
  // fp4 activations take the direct path (no LUT) and the output is bf16.
...
//
  ...
  if (errors == 0) {
    printf("fp4xfp6 asym WS matmul test PASSED (no mismatches).\n");
  } else {
    printf("fp4xfp6 asym WS matmul test FAILED with %d mismatches.\n", errors);
  }
