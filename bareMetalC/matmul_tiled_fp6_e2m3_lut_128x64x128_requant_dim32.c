// FP6 E2M3 (via LUT) 128x64x128 matmul, REQUANT output — DIM=32 (32x32 mesh), MxDim32AllGemminiRocketConfig.
// DIM=32 port of matmul_tiled_fp6_e2m3_lut_128x64x128_requant.c, same structure as the e4m3-lut dim32 requant
// test but fp6 format codes (A/B/OUT=1) + MX_ALTFMT=1 (E2M3) and 6-bit LUT entries. DIM-aware golden header.
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
#include "include/matmul_data_mx_lut_e2m3_128x64x128_dim32.h"

#define TILE 16
#define DIM 32
#define VALUES_PER_BYTE 2
#define USE_LUT 1
#define MX_ALTFMT 1                 // fp6 code1 + altfmt1 -> E2M3
...
int main() {
  ...
  // config_ex: E2M3 = fp6/code1 (A/B/OUT format 1) + altfmt1 + lut_en. 4-bit LUT indices in and out.
  ...
  if (errors == 0 && scale_errors == 0)
    printf("fp6 e2m3 (LUT) requant dim32 test PASSED (no mismatches).\n");
  else
    printf("fp6 e2m3 (LUT) requant dim32 test FAILED: %d code, %d scale mismatches.\n", errors, scale_errors);
