...
int main() {
  ...
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,
    ...
    | ((uint64_t)(1) << 12)         // B (weight) format = fp6 (code1); altfmt=1 -> E2M3 (quad/LUT only)
  ...
  if (errors == 0) printf("fp6 e2m3 (quad/LUT) WS matmul test PASSED (no mismatches).\n");
  else             printf("fp6 e2m3 (quad/LUT) WS matmul test FAILED with %d mismatches.\n", errors);
  return errors == 0 ? 0 : 1;
}
