// FP6 E2M3 (via LUT) 64x64 matmul, REQUANT output — DIM=32 (32x32 mesh), MxDim32AllGemminiRocketConfig.
// DIM=32 port of matmul_tiled_fp6_e2m3_lut_64x64_requant.c, same structure as the e4m3-lut dim32 requant
...
#include "include/matmul_data_mx_lut_e2m3_64x64_dim8.h"
...
int main() {
  ...
  if (errors == 0 && scale_errors == 0)
    printf("fp6 e2m3 (LUT) requant dim8 test PASSED (no mismatches).\n");
  else
    printf("fp6 e2m3 (LUT) requant dim8 test FAILED: %d code, %d scale mismatches.\n", errors, scale_errors);
