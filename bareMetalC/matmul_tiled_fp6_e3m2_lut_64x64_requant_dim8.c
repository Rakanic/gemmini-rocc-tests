...
#include "include/matmul_data_mx_lut_e3m2_64x64_dim8.h"
...
int main() {
  ...
  if (errors == 0 && scale_errors == 0)
    printf("fp6 e3m2 (LUT) requant dim8 test PASSED (no mismatches).\n");
  else
    printf("fp6 e3m2 (LUT) requant dim8 test FAILED: %d code, %d scale mismatches.\n", errors, scale_errors);
