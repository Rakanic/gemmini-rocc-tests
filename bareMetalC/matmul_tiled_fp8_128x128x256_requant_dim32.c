...
#include "include/matmul_fp8_128x128x256_dim32.h"
...
int main() {
  ...
  uint32_t scale_factors[MATMUL_M * MATMUL_GN / 4 + 1] __attribute__((aligned(32))) = {0};
  ...
  // ---- MVIN A: tile (i,k) -> a_base + (i*tiles_K + k)*DIM   (A stored [M][K], row stride MATMUL_K) ----
  ...
  // ---- MVIN B: tile (k,j) -> b_base + (k*tiles_J + j)*DIM   (B stored [K][N], row stride MATMUL_N) ----
  ...
  // Output spad region sits right after A (a_base + A's rows), clear of A reads and B (top of spad).
...
#if defined(SPIKE_SIM) || defined(MX_ROCKET)
  // V1 readback: requant FP8 output lives in the INTERNAL scratchpad; drain it spad->DRAM. spad reads
  // are full-width (DIM FP8/row), NOT chunked like the acc path.
  gemmini_fence();
  gemmini_config_st(DIM * sizeof(uint8_t));               // one spad row = DIM FP8 bytes
  uint8_t *c_base = (uint8_t *) C_hw;
  int total_spad_rows = MATMUL_M * MATMUL_N / DIM;
...
// ---- Elementwise check against C_out (fp8, byte-by-byte) ----
  ...
  for (int i = 0; i < MATMUL_M; i++) {
    for (int j = 0; j < MATMUL_N; j++) {
      ...
      if (got != exp) {
        errors++;
        if (errors <= 10) printf("Output[%d][%d], Got: %x, Exp: %x\n", i, j, got, exp);
