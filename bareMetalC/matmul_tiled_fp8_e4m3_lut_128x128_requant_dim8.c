// FP8 E4M3-quad (via LUT) 128x128 matmul, REQUANT output — DIM=8 (8x8 mesh), MxDim8AllAsymGemminiRocketConfig.
// 128x128 sibling of matmul_tiled_fp8_e4m3_lut_64x64_requant_dim8.c: same DIM=8 quad-LUT requant path, size
// grows to stress the requant read throttle / drain queue with more back-to-back mvout bursts. Prior header:
...
#include "include/matmul_data_mx_lut_e4m3_128x128_dim8.h"
...
int main() {
  ...
  if (errors == 0 && scale_errors == 0)
    printf("fp8 e4m3-quad requant dim8 test PASSED (no mismatches).\n");
  else
    printf("fp8 e4m3-quad requant dim8 test FAILED: %d code, %d scale mismatches.\n", errors, scale_errors);
