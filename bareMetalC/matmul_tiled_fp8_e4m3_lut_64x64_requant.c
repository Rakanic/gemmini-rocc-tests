// FP8 E4M3-quad (via LUT) 64x64 matmul, REQUANT output — standalone MxE5M2GemminiRocketConfig.
// Same as the operand test, but the BF16 result is requantized to E4M3-quad 4-bit LUT indices in the
...
int main() {
  ...
  // config_ex: symmetric encoding — E4M3-quad = fp8/code0 + altfmt0 + lut_en. A/B/OUT format = 0 (fp8), altfmt=1,
  ...
  if (errors == 0 && scale_errors == 0)
    printf("fp8 e4m3-quad requant test PASSED (no mismatches).\n");
  else
    printf("fp8 e4m3-quad requant test FAILED: %d code, %d scale mismatches.\n", errors, scale_errors);
