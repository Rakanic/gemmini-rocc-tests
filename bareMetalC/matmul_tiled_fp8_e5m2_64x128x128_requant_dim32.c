// FP8 E5M2 (via LUT) 64x128x128 matmul, REQUANT output — DIM=32 (32x32 mesh), MxDim32AllGemminiRocketConfig.
// DIM=32 port of matmul_tiled_fp8_e5m2_64x128x128_requant.c, same structure as the e4m3-lut dim32 requant test
// but MX_ALTFMT=1 (E5M2 codebook). Uses the DIM-aware Spike-consistent golden header (gen_mx_lut.py --dim 32).
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
#include "include/matmul_data_mx_lut_e5m2_64x128x128_dim32.h"

#define TILE 16
#define DIM 32
#define VALUES_PER_BYTE 2
#define USE_LUT 1
#define MX_ALTFMT 1                 // E5M2 (fp8 code0 + altfmt1)
...
int main() {
  ...
  // config_ex: E5M2 = fp8/code0 (A/B/OUT format 0) + altfmt1 + lut_en. 4-bit LUT indices in and out.
  ...
  int SPAD_DEST = tiles_I * tiles_K * DIM;   // clear the A footprint (shape-general)
  ...
  if (errors == 0 && scale_errors == 0)
    printf("fp8 e5m2 (LUT) requant dim32 test PASSED (no mismatches).\n");
  else
    printf("fp8 e5m2 (LUT) requant dim32 test FAILED: %d code, %d scale mismatches.\n", errors, scale_errors);
