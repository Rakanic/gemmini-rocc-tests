// FP8 E5M2 (via LUT) 64x64 matmul, REQUANT output — DIM=32 (32x32 mesh), MxDim32AllGemminiRocketConfig.
// DIM=32 port of matmul_tiled_fp8_e5m2_64x64_requant.c, same structure as the e4m3-lut dim32 requant test
...
#include "include/matmul_data_mx_lut_e5m2_64x64_dim8.h"
...
int main() {
  ...
  int SPAD_DEST = tiles_I * tiles_K * DIM;   // clear A footprint (dim8)
  ...
  if (errors == 0 && scale_errors == 0)
    printf("fp8 e5m2 (LUT) requant dim8 test PASSED (no mismatches).\n");
  else
    printf("fp8 e5m2 (LUT) requant dim8 test FAILED: %d code, %d scale mismatches.\n", errors, scale_errors);
