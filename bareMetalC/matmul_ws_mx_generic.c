...
int main() {
  ...
  // for (size_t m = 0; m < MATMUL_M; m += MATMUL_M) { // TODO: 32 for fp6/4
  //   for (size_t n = 0; n < MATMUL_N; n += MATMUL_M) {
  //     for (size_t k = 0; k < MATMUL_K; k += TILE) {
  //       if (k % 32 == 0) {
load_scale_factors((volatile uint64_t *) GEMMINI_SF_MEM_A, (uint8_t *) &A_scales_row, 32);
load_scale_factors((volatile uint64_t *) GEMMINI_SF_MEM_A, (uint8_t *) &A_scales_row, 32);
load_scale_factors((volatile uint64_t *) GEMMINI_SF_MEM_B, (uint8_t *) &B_scales_col, 32);
load_scale_factors((volatile uint64_t *) GEMMINI_SF_MEM_B, (uint8_t *) &B_scales_col, 32);
