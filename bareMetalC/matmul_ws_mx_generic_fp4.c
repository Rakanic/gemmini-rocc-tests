...
#include "include/matmul_data_fp4_direct.h"
...
int main() {
  ...
  static out_t C_hw[MATMUL_M/2][MATMUL_N/8];
  ...
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,
    ...
    | (0),
    k_CONFIG);
  //gemmini_extended3_config_ex(WEIGHT_STATIONARY, 0, 0, ACC_SCALE_IDENTITY, 1, 1, 0, 0, false, GEMMINI_FORMAT, GEMMINI_FORMAT, GEMMINI_FORMAT, 0);
  ...
  gemmini_mxquant_config_mvout((uint64_t)scale_factors, 1, tiles_I, tiles_J, tiles_K, 0, 0, QUANT_LUT_UPDATE_GRANULARITY);
   // MVIN B
...
// gemmini_compute_preloaded(0 * DIM, GARBAGE_ADDR);
  ...
  for (int i = 0; i < MATMUL_M; i ++) {
    for (int j = 0; j < MATMUL_N / 8; j++) {
        ...
        printf("Elem: %d = %lx \n", i * MATMUL_M + j, *(smem_start_addr + (i*MATMUL_N / 8 + j)));
