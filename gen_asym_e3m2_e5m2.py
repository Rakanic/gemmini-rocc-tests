#!/usr/bin/env python3
"""Generate the data header for the ASYMMETRIC FP6_E3M2-activation x FP8_E5M2-weight matmul (mode4).

BOTH operands are LUT formats (both deproject):
  A (activation) is FP6_E3M2 -> 4-bit LUT indices + per-row-group E3M2 codebook (6-bit codes).
  B (weight) is FP8_E5M2 -> 4-bit LUT indices + per-col-group E5M2 codebook (8-bit codes).
Both codebooks are LOADED at 8-bit entries (E3M2's 6-bit codes zero-extended, high 2 bits = 0;
RTL/spike slice the low 6). Output is BF16 (compared against C_out_bf16).
...
ACT_SPEC = "fp6:e3m2"     # FP6 E3M2 activation (LUT, 6-bit codes, loaded 8-bit-padded)
WEI_SPEC = "fp8:e5m2"     # FP8 E5M2 weight (LUT, 8-bit codebook)
SCALE_SPEC = "fpe8m0"
LUT_INDEX_BITS = 4
...
A_TILE_M = 32
K_TILE   = 16
HEADER   = os.environ.get("ASYM_HEADER_PATH", "./include/matmul_data_asym_e3m2_e5m2.h")
...
# Codebook code values (E3M2 codes are 6-bit; E5M2 codes are 8-bit).
A_lut_codes, _ = tensor_to_custom_fp_codes(A_luts_t, ACT_SPEC)       # (M>>G, 16), 6-bit codes
B_lut_codes, _ = tensor_to_custom_fp_codes(B_luts_t, WEI_SPEC)       # (N>>G, 16), 8-bit codes
# Both LUTs are LOADED at 8-bit entries: the RTL LUT stores 8-bit slots and slices the low 6 for
# E3M2 at deproject time, so each 6-bit E3M2 code is zero-extended into an 8-bit slot (high 2 = 0).
...
with open(HEADER, "w") as f:
    ...
    f.write("// A (activation) = FP6_E3M2 4-bit LUT indices, HW-tiled [M/2][K].\n")
    ...
    f.write(f"// A_lut = E3M2 codebook, 6-bit codes zero-extended to {A_LUT_ENTRY_BITS}-bit slots "
            f"(RTL slices low 6) -> {A_LUT_WORDS}x uint32. Loaded (sel=1).\n")
    f.write(f"static const uint32_t A_lut[{n_a}][{A_LUT_WORDS}] = {{\n{A_lut_packed}\n}};\n\n")
    f.write(f"// B_lut = E5M2 codebook (16x{B_LUT_ENTRY_BITS}-bit -> {B_LUT_WORDS}x uint32). Loaded (sel=0).\n")
