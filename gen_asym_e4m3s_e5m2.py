#!/usr/bin/env python3
"""ASYMMETRIC FP8_E4M3-SINGLE-activation x FP8_E5M2-weight matmul (mode7, dual throughput 16x32).

E4M3 activation is DIRECT (single, 1 row/lane, no act LUT); E5M2 weight is quad (LUT-deprojected, 8-bit codes,
...
(no activation LUT) even though a WEIGHT LUT is loaded (mx_lut_b_loaded=1).
  ...
  B (weight)     = FP8_E5M2 4-bit LUT indices, nibble-packed [K][N/2] + per-col-group codebook. TN=32.
...
WEI_SPEC = "fp8:e5m2"     # FP8_E5M2 weight (LUT, 8-bit codes)
SCALE_SPEC = "fpe8m0"
LUT_INDEX_BITS = 4
GROUP    = 32
G        = 1              # QUANT_LUT_UPDATE_GRANULARITY
HEADER   = os.environ.get("ASYM_HEADER_PATH", "./include/matmul_data_asym_e4m3s_e5m2.h")
...
B_lut_codes, _ = tensor_to_custom_fp_codes(B_luts_t, WEI_SPEC)       # (N>>G, 16), 8-bit E5M2 codes
B_LUT_ENTRY_BITS = 8   # 8-bit E5M2 codes fill the byte (weiCodeWidth=8)
...
with open(HEADER, "w") as f:
    ...
    f.write(f"// B_lut = E5M2 codebook, 8-bit codes in {B_LUT_ENTRY_BITS}-bit slots -> "
...
print(f"Wrote {HEADER}  (M={M} K={K} N={N}, Gk={Gk}, E4M3-single act x E5M2 quad wei -> mode7)")
