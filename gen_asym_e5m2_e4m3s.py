...
E5M2 activation is quad (2 rows/lane, LUT-deprojected); E4M3 weight is DIRECT single (1 col/lane, no
...
ACT_SPEC = "fp8:e5m2"     # FP8_E5M2 activation (LUT, quad, 8-bit codes)
...
A_TILE_M = 32
K_TILE   = 16
HEADER   = os.environ.get("ASYM_HEADER_PATH", "./include/matmul_data_asym_e5m2_e4m3s.h")
...
A_lut_codes, _ = tensor_to_custom_fp_codes(A_luts_t, ACT_SPEC)       # (M>>G, 16), 8-bit E5M2 codes
A_LUT_ENTRY_BITS = 8   # 8-bit E5M2 codes fill the byte (actCodeWidth=8)
...
with open(HEADER, "w") as f:
    ...
    f.write(f"// A_lut = E5M2 codebook, 8-bit codes in {A_LUT_ENTRY_BITS}-bit slots -> "
...
print(f"Wrote {HEADER}  (M={M} K={K} N={N}, Gk={Gk}, E5M2 quad act x E4M3-single wei -> mode5)")
