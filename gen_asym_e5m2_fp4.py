#!/usr/bin/env python3
"""Generate the data header for the ASYMMETRIC FP8_E5M2-activation x FP4-weight matmul test (mode3).

Same as gen_asym_fp6_fp4.py but the activation format is FP8_E5M2 instead of FP6_E3M2:
  A (activation) is FP8_E5M2 stored as 4-bit LUT indices + a per-row-group E5M2 codebook (8-bit
    codes, activation LUT deprojects it). A_in_hw = E5M2 indices in the HW-tiled layout.
...
ACT_SPEC = "fp8:e5m2"     # FP8 E5M2 activation (LUT-deprojected, 8-bit codebook)
...
A_TILE_M = 32
K_TILE   = 16
HEADER   = os.environ.get("ASYM_HEADER_PATH", "./include/matmul_data_asym_e5m2_fp4.h")
...
with open(HEADER, "w") as f:
    ...
    f.write("// A (activation) = FP8_E5M2 4-bit LUT indices, HW-tiled [M/2][K].\n")
