#!/usr/bin/env python3
"""Generate the data header for the ASYMMETRIC FP4-activation x FP8_E5M2-weight matmul test (mode1).

Same as gen_asym_fp4_fp6.py but the weight format is FP8_E5M2 instead of FP6_E3M2:
  A (activation) is FP4 E2M1 stored as DIRECT 4-bit codes (fed straight to the mesh, no LUT).
  B (weight) is FP8_E5M2 stored as 4-bit LUT indices + a per-col-group E5M2 codebook (8-bit codes,
    LUT-deprojected).
...
WEI_SPEC = "fp6:e2m3"     # FP6 E2M3 weight (LUT, 6-bit codebook)
SCALE_SPEC = "fpe8m0"
LUT_INDEX_BITS = 4
...
A_TILE_M = 32
K_TILE   = 16
HEADER   = os.environ.get("ASYM_HEADER_PATH", "./include/matmul_data_asym_fp4_e2m3.h")
...
with open(HEADER, "w") as f:
    ...
    f.write("// B (weight) = FP6_E2M3 4-bit LUT indices, nibble-packed [K][N/2] (odd col high nibble)\n")
