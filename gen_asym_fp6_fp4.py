#!/usr/bin/env python3
"""Generate the data header for the ASYMMETRIC FP6_E3M2-activation x FP4-weight matmul test (mode3).

Mirror of gen_asym_fp4_fp6.py with A and B roles SWAPPED:
  A (activation) is FP6_E3M2 stored as 4-bit LUT indices + a per-row-group fp6 codebook (the
    activation LUT deprojects it). A_in_hw = fp6 indices in the HW-tiled layout.
...
ACT_SPEC = "fp6:e3m2"     # FP6 E3M2 activation (LUT-deprojected)
...
A_TILE_M = 32
K_TILE   = 16
HEADER   = os.environ.get("ASYM_HEADER_PATH", "./include/matmul_data_asym_fp6_fp4.h")
