...
GRID = N // BLOCK   # 4x4 grid of 32x32 tiles -> 128x128

#: Real 32x32 tiles stitched into a GRIDxGRID grid; only 8 distinct tiles are logged, so cycle them.
LAYER = "layer0"
PROJ = "mlp.gate_proj"
N_TILES = 8
A_TILES = [f"A_square_{i % N_TILES:03d}.npz" for i in range(GRID * GRID)]
W_TILES = [f"W_square_{i % N_TILES:03d}.npz" for i in range(GRID * GRID)]
...
PROD_PRECISION = [(4, 3)] * 8   # DIM=8 mesh depth
ACC_PRECISION = [(8, 7)] * 8    # DIM=8: all rows bf16

DATA = MXQ_ROOT / "end_to_end_linear" / "systolic_simulation" / "data_evalrun_01"


def _stitch(names: list[str]) -> np.ndarray:
    """GRID*GRID real 32x32 tiles -> one (GRID*32)x(GRID*32), row-major grid."""
    parts = []
    for n in names:
        p = DATA / LAYER / PROJ / n
        if not p.exists():
        ...
        parts.append(t)
    rows = [np.concatenate(parts[r * GRID:(r + 1) * GRID], axis=1) for r in range(GRID)]
...
def main() -> None:
    ...
    hdr = HERE / "include" / "matmul_fp8_128x128_dim8.h"
...
#ifndef INCLUDE_MATMUL_FP8_128X128_DIM8_H
#define INCLUDE_MATMUL_FP8_128X128_DIM8_H
...
#endif // INCLUDE_MATMUL_FP8_128X128_DIM8_H
""")
    print(f"wrote {hdr}")
