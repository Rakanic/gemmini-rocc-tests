...
def _a_indices_to_hw_layout(indices, a_tile_m: int = 32, k_tile: int = 16) -> list:
    """Convert [M, K] index tensor/list to HW tiled layout [M//2, K].
    Within each (a_tile_m x k_tile) block, pairs of M-rows are interleaved per K column.
    Output byte: bits[7:4]=A[2r+1, k], bits[3:0]=A[2r, k] for hw-row r."""
    if isinstance(indices, Tensor):
        data = indices.tolist()
    else:
        data = [list(r) for r in indices]
    M = len(data)
    K = len(data[0]) if M > 0 else 0
    out = [[0] * K for _ in range(M // 2)]
        ...
        for ki in range(0, K, k_tile):
            for r in range(a_tile_m // 2):
                ...
                for c in range(k_tile):
                    col = ki + c
                    even = data[mi + 2*r][col] & 0xF
                    odd  = data[mi + 2*r + 1][col] & 0xF
                    out[hw_row][col] = (odd << 4) | even
    return out


def write_c_header_tiled_hw(
    path: str,
    M: int, K: int, N: int,
    group: int,
    input_spec: str,
    ...
    """Write C header with HW tiled layout for A and pre-packed LUT words."""
    Gk = K // group
    M_hw = M // 2

    # A: HW tiled layout [M//2, K], nibble-packed pairs of m-rows
    A_hw = _a_indices_to_hw_layout(A_in, a_tile_m, k_tile)

    # B: standard nibble-packed [K, N//2], odd-col index in high nibble
    B_codes = B_in.tolist()
    B_hw = [[(r[i+1] << 4) | r[i] for i in range(0, len(r), 2)] for r in B_codes]

    # C_out: standard nibble-packed [M, N//2]
    Cq_codes = C_out_quantized.tolist()
    C_out_hw = [[(r[i+1] << 4) | r[i] for i in range(0, len(r), 2)] for r in Cq_codes]
    ...
    As_bits -= 1; Bs_bits -= 1; Cqs_bits -= 1
    ...
    if lut_index_bits >= 0 and A_lut is not None:
        A_lut_codes, _ = tensor_to_custom_fp_codes(A_lut, input_spec)  # [n_a_groups][16]
        B_lut_codes, _ = tensor_to_custom_fp_codes(B_lut, input_spec)  # [n_b_groups][16]
        C_lut_codes, _ = tensor_to_custom_fp_codes(C_lut, input_spec)  # [n_c_groups][16]
    ...
    with open(path, "w") as f:
        ...
        f.write(f"// Input precision: {input_spec} ({lut_index_bits}-bit LUT indices)\n")
        f.write(f"// A stored in HW tiled layout: per (A_TILE_M x K_TILE) block,\n")
        f.write(f"//   pairs of m-rows interleaved per k, then adjacent column pairs nibble-packed.\n")
        f.write(f"//   Dimensions: [M/2][K] = [{M_hw}][{K}], each byte = two 4-bit indices\n")
        f.write(f"//   byte layout: bits[7:4]=a(2r+1, k), bits[3:0]=a(2r, k)  for hw-row r\n")
        f.write(f"static const uint8_t A_in_hw[{M_hw}][{K}] = {{\n{fmt2d(A_hw)}\n}};\n\n")

        f.write(f"// B stored in standard layout: odd-col nibble in high bits\n")
        ...
        f.write(f"static const uint8_t C_out[MATMUL_M][MATMUL_N / 2] = {{\n{fmt2d(C_out_hw)}\n}};\n\n")
        ...
        f.write(f"// Final output (scaled+accumulated), bf16\n")
